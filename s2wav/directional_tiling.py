...
def logfact(n: int) -> float:
    r"""Computes the natural logarithm of an integer factorial.
...
def binomial_coefficient(exact: bool, n: int, k: int) -> int:
    ...
    if not exact:
        ...
        return math.floor(
            0.5 + math.exp(loggamma(n + 1) - loggamma(k + 1) - loggamma(n - k + 1))
        )
    ...
    for el in range(1, L):
        ...
        for m in range(-el, el):
            ...
            if abs(m) < N and (N + m) % 2:
                s_elm[ind] = nu * math.sqrt(
                    (binomial_coefficient(gamma, int((gamma - m) / 2), 1)) / 2**gamma
                )
            else:
                s_elm[ind] = 0.0
...
def tiling_wavelet(
    ...
    for j in range(J_min, J):
        ...
        for el in range(el_min, L):
            for m in range(-el, el):
                psi[j * L * L + ind] = (
                    math.sqrt((2 * el + 1) / (8.0 * math.pi * math.pi))
                    * kappa[j * L + el]
                    * s_elm[ind]
                )
                if original_spin != 0:
                    psi[j * L * L + ind] *= (
                        spin_normalization(el, original_spin) * (-1) ** original_spin
                    )
                ind += 1
